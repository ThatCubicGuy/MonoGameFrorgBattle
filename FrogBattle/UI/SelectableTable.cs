...
namespace FrogBattle.UI
{
	public class SelectableTable : IGameEntity, IEnumerable<ISelectableCell>
	{
		...
		public SelectableTable(ISelectableCell[] cells, ISelectableCell defaultValue, Point grid_size, Point location, Point cell_size, Point cell_spacing = new())
		{
			...
			cellLocations = new Point[BoxCount];
			this.cells = new ISelectableCell[BoxCount];
			actualCellCount = cells.Length;
			Array.Copy(cells, this.cells, actualCellCount);
			Array.Fill(this.cells, defaultValue, actualCellCount, BoxCount - actualCellCount);
			for (int i = 0; i < Rows; i++)
			{
				for (int j = 0; j < Columns; j++)
				{
					// Create a static list of the locations of each box, relative to the table's location.
					cellLocations[i * Columns + j] = new Point((cellSize.X + cellSpacing.X) * j, (cellSize.Y + cellSpacing.Y) * i);
				}
			}
        }
		...
		public int BoxCount => Rows * Columns;
